...
function BodyProducts({products}:{products:typeProduct[]|undefined}) {
  ...
  return (
    <div className="w-full min-h-screen mt-[60px] mb-1 p-6 flex flex-wrap justify-around gap-y-5 gap-x-6 items-center bg-white border border-solid border-black">
          {/* //    {/*************************************** cards The products **************************/}
    {products && products.map(({id,title_pro,images}:typeProduct,productsIndex:number)=>{
     ...
     return( <div className='sm:w-full w-72 h-full border-0 border-solid border-red-900'  key={productsIndex}>
          ...
          <Link href={`/productdetails/${id}?imageIndex=${CurrentSelectIndex}`}  className=" sm:w-full block rounded-lg p-4 shadow-sm shadow-indigo-100 border hover:shadow-md hover:border hover:border-solid hover:border-indigo-300 hover:rounded-lg ">
